...
import { File, Download, Upload, CheckCircle, X } from "lucide-react";
...
export default function DocumentsPage() {
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          ...
          <button
            ...
            className="flex items-center gap-2 px-5 py-2.5 bg-[#0f172a] text-white rounded-xl hover:bg-[#1e293b] transition-all text-xs md:text-sm font-medium self-start"
          >
      ...
      {/* Category Pills */}
      <motion.div
        className="flex gap-2 overflow-x-auto pb-2 mb-6 -mx-4 md:-mx-8 px-4 md:px-8"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
          ...
          {filtered.map((doc, idx) => (
            ...
            >
              <div className="flex items-start justify-between mb-4">
                ...
                <span className="text-[10px] md:text-xs text-slate-400 bg-slate-50 px-2 py-1 rounded-full">
                  {doc.category}
                </span>
              </div>
